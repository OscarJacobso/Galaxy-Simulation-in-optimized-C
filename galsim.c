...
f force_calc(struct part particlesi, struct part particlesj){           /*Takes two particles as input and returns the force in x and y direction*/ 
    struct fxy f;
    double Fx;
    double Fy;
    double distx = particlesi.Posx -particlesj.Posx;
    double disty = particlesi.Posy -particlesj.Posy;
    double rad = sqrt(distx*distx + disty*disty);                       /*Radius*/

    /*printf("Rad %f\n", rad);
    printf("Disty %f\n", disty);*/
    /*printf("Pow %f\n", pow((rad+epsil),3));
    printf("Pow %f\n", (rad+epsil)*(rad+epsil)*(rad+epsil));*/                                     
    Fx = -G*particlesi.Mass * particlesj.Mass/pow((rad+epsil),3) * distx;
    Fy = -G*particlesi.Mass * particlesj.Mass/pow((rad+epsil),3) * disty;
...
void pos_update(struct part *particles, double ax, double ay){             /*Takes pointer to particle and updates its position given acceleration in x and y plane*/
    particles->Velx = particles->Velx + delta*ax;
    particles->Vely = particles->Vely + delta*ay;
    /*printf("Vx = %f Vy = %f\n", particles->Velx, particles->Vely);*/

    double px = particles->Posx + delta*particles->Velx;
    double py = particles->Posy + delta*particles->Vely;            /*If particles fly out they get put on the edge of the 1 x 1 box*/
    if (px >= 1){
        particles->Posx = 1;
    }
    else if(px <= 0){
...
int main(int argc, char** argv){
    if (6 != argc){
        ...
        double *accx = (double *)malloc(N * sizeof(double));      /*Malloc memory for N size array ix */
        double *accy = (double *)malloc(N * sizeof(double));      /*Malloc memory for N size array iy */
        double sumx, sumy;
        struct fxy ftot;
        ...
        for (int n = 0; n < stp; n++){
            for (int i = 0; i < N; i++){
                ...
                for (int j = 0; j < N; j++){
                    if (i != j){
                        ftot = force_calc(particles[i], particles[j]);                /*Force calc to get force in x and y direction for particle i*/  /*If for a matrix: (*arr is first value, *arr+i*N is ith column(or row idk), *arr+i*N+j is jth element of ith row*/
                        sumx = sumx + ftot.fx;
                        sumy = sumy + ftot.fy;
                    }
                }
                /*printf("Sum of x forces %f Sum of y forces %f\n\n", sumx, sumy);*/
                *(accx + i) = sumx/particles[i].Mass;                                         /*sum accelerations in every column*/
                *(accy + i) = sumy/particles[i].Mass;
            }
            /*for (int i = 0; i < N; i++){
            ...
            for (int i = 0; i < N; i++){
                ...
                pos_update(&particles[i], accx[i], accy[i]);
                if (graph == 1 && quit != 1){
