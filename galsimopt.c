...
void force_calc(struct part *particles, double *ax, double *ay){           /*Takes two particles as input and returns the force in x and y direction*/ 
    ...
    for (int i = 0; i < N; i++){
        ...
        for (int j = 0; j < N; j++){
            if (i != j){          
                ...
                sumx = sumx + Fx;                                                                       /*Add forces to the total force on i*/
                sumy = sumy + Fy;
            }
        }
        *(ax + i) = sumx/(particles + i)->Mass;                                                       /*Total x-force from every particle on i gives its acceleration, stored in accx*/                                                    
        *(ay + i) = sumy/(particles + i)->Mass;                                                       /*Total Y-force from every particle on i gives its acceleration, stored in accy*/ 
    }
    /*printf("Force calc\nFx = %f Fy = %f\n", f.fx, f.fy);*/
}
...
void pos_update(struct part *particles, double *ax, double *ay){             /*Takes pointer to particle and updates its position given acceleration in x and y plane*/
    for (int i = 0; i < N; i++){
        ...
        particles[i].Velx = Vx + delta*x;
        particles[i].Vely = Vy + delta*y;
...
int main(int argc, char** argv){
    if (6 != argc){
        ...
        for (int n = 0; n < stp; n++){
            force_calc(particles, accx, accy);                /*Force calc to get force in x and y direction for particle i*/  /*If for a matrix: (*arr is first value, *arr+i*N is ith column(or row idk), *arr+i*N+j is jth element of ith row*/
            ...
            /* step all particles*/
            pos_update(particles, accx, accy);
